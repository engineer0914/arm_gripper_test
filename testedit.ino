      ...
      if (abs_c >= GRASP_CURRENT_THRESHOLD)
      {
        ...
        else
        {
          ...
          if ((millis() - grasp_start_ms) >= GRASP_HOLD_MS)
          {
            ...
            DEBUG_PORT.println("[RESULT] GRASP_OK");
            return;
          }
        }
      }
...
//  DEBUG_PORT.println("[GRASP] Failed (timeout or no condition). Releasing...");
  ...
  DEBUG_PORT.println("[RESULT] GRASP_FAIL");
}
...
// Python에서 ser.write(b'grip\n'), ser.write(b'open\n') 받기용
// 기존 동작 함수는 그대로 사용:
//   grip / close / 1  -> rhp12GraspByCurrent()
//   open / 0          -> rhp12OpenAndTorqueOff()
// ============================================================
String serial_cmd = "";
...
void handleSerialCommand(String cmd)
{
  ...
  if (cmd == "grip" || cmd == "close" || cmd == "grasp" || cmd == "1")
  {
    DEBUG_PORT.println("[SERIAL] GRIP command received");
    rhp12GraspByCurrent();
  }
  else if (cmd == "open" || cmd == "0")
  {
    DEBUG_PORT.println("[SERIAL] OPEN command received");
    rhp12OpenAndTorqueOff();
  }
  else
  {
    DEBUG_PORT.print("[WARN] Unknown command: ");
    DEBUG_PORT.println(cmd);
  }
}
...
void loop()
{
  while (DEBUG_PORT.available())
  {
    ...
    // 줄바꿈이 오면 현재까지 받은 문자열 실행
    if (c == '\n' || c == '\r')
    {
      if (serial_cmd.length() > 0)
      {
        handleSerialCommand(serial_cmd);
        serial_cmd = "";
      }
    }
    // 기존 방식 유지: 1, 0은 한 글자만 와도 즉시 실행
    else if (c == '1' || c == '0')
    {
      String one_char_cmd = "";
      one_char_cmd += c;

      handleSerialCommand(one_char_cmd);
      serial_cmd = "";
    }
    // 문자열 명령 누적
    else
    {
      ...
      // close, grip, grasp, open이 완성되면 줄바꿈 없어도 즉시 실행
      if (
        serial_cmd == "close" ||
        serial_cmd == "grip"  ||
        serial_cmd == "grasp" ||
      ...
      // 이상한 문자열이 계속 들어올 때 버퍼 초기화
      if (serial_cmd.length() > 32)
      {
        DEBUG_PORT.println("[WARN] Serial command buffer overflow. Cleared.");
        serial_cmd = "";
      }
    }
  }
