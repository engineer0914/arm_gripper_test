...
// ============================================================
// Mega2560 + Dynamixel Shield 설정
//  - DEBUG: Serial (USB)
//  - DXL BUS: Serial1 (TX1=18, RX1=19)
//  - Half-duplex DIR pin: DXL_DIR_PIN (보통 D2, 실드 설정에 따라 변경)
// ============================================================
#define DEBUG_PORT   Serial3
#define DXL_PORT     Serial

#define DXL_DIR_PIN  2   // ★ Dynamixel Shield의 DIR 핀 (필요시 변경)

// DIR 제어: 일반적으로 HIGH=TX, LOW=RX
static inline void dxlTxEnable(bool enable_tx)
{
  digitalWrite(DXL_DIR_PIN, enable_tx ? HIGH : LOW);

  // 만약 통신이 전혀 안 되면 아래처럼 반대로 바꿔서 테스트:
  // digitalWrite(DXL_DIR_PIN, enable_tx ? LOW : HIGH);
}
...
// 목표 위치(예시). 하드스톱에 걸면 떨림이 커지므로 끝단에서 약간 빼서 시작
#define OPEN_POS               30
#define CLOSE_POS              710

// 전류(단위: RH-P12 control table unit)
...
// ============================================================
// Half-duplex TX/RX helpers (Mega + Dynamixel Shield)
// ============================================================
void dxlFlushRx()
{
...
int dxlReceive(uint8_t *rx, int rx_max, uint32_t timeout_ms)
{
  ...
  while ((millis() - t0) < timeout_ms)
  {
    ...
    // Protocol 2.0: 최소 11~12바이트 이상부터 length 파싱 가능
    if (rx_len >= 12)
    {
      uint16_t len = (uint16_t)rx[5] | ((uint16_t)rx[6] << 8);
      int total = 7 + (int)len; // header(4)+id(1)+len(2) = 7, + len
      if (rx_len >= total)
      {
        return rx_len;
      }
...
{
  uint16_t len = (uint16_t)(1 + param_len + 2); // INST(1) + PARAM + CRC(2)
...
{
  if (!dxlBuildAndSend(id, INST_PING, nullptr, 0)) return false;
...
bool dxlRead(uint8_t id, uint16_t addr, uint16_t size, uint8_t *out, uint16_t out_max)
{
  ...
  uint16_t len = (uint16_t)rx[5] | ((uint16_t)rx[6] << 8);
  uint16_t params_len = (uint16_t)(len - 1 - 1 - 2); // INST(1) + ERR(1) + CRC(2) 제외

  if (params_len < size) return false;
  if (size > out_max) return false;

  // status packet: [0..3]=header, [4]=id, [5..6]=len, [7]=0x55, [8]=err, [9..]=params
...
// OPEN 동작: 목표 도달 후 떨림 완화 + Torque OFF
...
void setup()
{
  ...
  // Mega는 보통 while(!Serial) 필요 없음(있어도 큰 문제는 없음)
  // while (!DEBUG_PORT) { ; }

  pinMode(DXL_DIR_PIN, OUTPUT);
  dxlTxEnable(false); // RX 모드로 시작

  DXL_PORT.begin(DXL_BAUD);

  delay(100);

  DEBUG_PORT.println("=== RH-P12-RN(A) Current-based Grasp (MEGA + DXL Shield) ===");
  ...
  DEBUG_PORT.println("  '0' : OPEN and Torque OFF");
